...
FU_MANAGER_DEPS = verilog/mult.sv verilog/alu.sv
$(call DEPS,fu_manager): $(FU_MANAGER_DEPS)

RS_DEPS = verilog/psel_gen.sv
$(call DEPS,rs): $(RS_DEPS)

DISPATCH_STAGE_DEPS = verilog/free_list.sv
$(call DEPS,dispatch_stage): $(DISPATCH_STAGE_DEPS)

FETCH_DEPS = verilog/icache.sv verilog/inst_buffer.sv verilog/branch_predictor.sv test/mem.sv
$(call DEPS,fetch): $(FETCH_DEPS)

# LOAD_STORE_QUEUE_DEPS = verilog/dcache.sv
# $(call DEPS,load_store_queue): $(LOAD_STORE_QUEUE_DEPS)

DISPATCH_STAGE_DEPS = verilog/free_list.sv
$(call DEPS,dispatch_stage): $(DISPATCH_STAGE_DEPS)

PIPELINE_DEPS = verilog/rob.sv verilog/dispatch_stage.sv verilog/fetch.sv verilog/rs.sv verilog/fu_manager.sv verilog/map_table.sv \
verilog/mmu.sv verilog/psel_gen.sv verilog/regfile.sv verilog/icache.sv verilog/inst_buffer.sv verilog/branch_predictor.sv test/mem.sv
...
# This is invoked in CI, it is used to do a basic compile via modelsim
local_check_compile:
	./vlog -svinputport=relaxed $(HEADERS) $(SOURCES)
